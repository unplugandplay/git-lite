...
module GitLite
  module CLI
    ...
    def self.run_commit(args)
      ...
      puts "[#{repo.current_branch} #{commit_id[0..7]}] #{message.lines.first.chomp}"
    end
    ...
    def self.run_stats(args)
      ...
      if stats[:content_versions]
        puts ""
        puts "Delta Compression"
        puts "-" * 30
        puts "Versions:     #{stats[:content_versions]}"
        puts "Keyframes:    #{stats[:content_keyframes]}"
        puts "Deltas:       #{stats[:content_deltas]}"
        if stats[:content_versions] > 0
          ratio = (stats[:content_deltas].to_f / stats[:content_versions] * 100).round(1)
          puts "Delta ratio:  #{ratio}%"
        end
      end
    ...
    def self.expand_paths(args)
      ...
      args.each do |arg|
        if arg == '.'
          files.concat(Dir.glob('**/*', File::FNM_DOTMATCH).select { |f| File.file?(f) })
        elsif arg.include?('*')
