...
class EdgeCasesTest < GitLite::TestCase
  ...
  def test_unicode_filename
    ...
    @repo.stage_file(name)
    commit = @repo.commit('Unicode filename')
    
    blob = @repo.db.get_blob(name, commit)
    assert_equal 'content', blob[:content]
  end
  ...
  def test_parallel_file_reads
    ...
    @repo.stage_file('test.txt')
    commit = @repo.commit('Test')
    
    # Simulate concurrent reads
    threads = 10.times.map do
      Thread.new do
        @repo.db.get_blob('test.txt', commit)
      end
    end
    
    results = threads.map(&:value)
    results.each { |r| assert_equal 'content', r[:content] }
  end
