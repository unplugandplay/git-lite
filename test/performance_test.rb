...
class PerformanceTest < GitLite::TestCase
  def test_commit_performance
    ...
    20.times do |i|
      ...
      times << (t_end.to_f - t_start.to_f)
    end

    avg_time = times.inject(0.0) { |sum, t| sum + t } / times.length
  ...
  def test_large_file_storage
    ...
    puts "\n  1MB file commit time: #{elapsed.round(4)}s"
  ...
  def test_many_small_files
    ...
    puts "\n  100 files commit time: #{elapsed.round(4)}s"
  ...
  def test_log_performance
    ...
    puts "\n  Log 50 from 100 commits time: #{elapsed.round(4)}s"
  ...
  def test_tree_traversal_performance
    ...
    puts "\n  Tree traversal (10x) time: #{elapsed.round(4)}s"
